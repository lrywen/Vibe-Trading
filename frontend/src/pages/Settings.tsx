...
export function Settings() {
    ...
    Promise.all([api.getLLMSettings(), api.getDataSourceSettings()])
      ...
      .catch((error) => {
        ...
        if (isAuthRequiredError(error)) {
          ...
          toast.error(`${t("common.serverError")}: ${message}`);
        }
      })
      .finally(() => {
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      ...
      toast.error(`${t("common.serverError")}: ${error instanceof Error ? error.message : t("common.unknownError")}`);
    } finally {
      setSaving(false);
    }
  };
  ...
  const localApiAccessSection = (
    <form onSubmit={submitLocalApiKey} className="rounded-lg border bg-card p-5 shadow-sm">
      ...
      <div className="grid gap-3 md:grid-cols-[minmax(0,1fr)_auto]">
        <label className="grid gap-2">
          ...
          <input
            ...
            placeholder={t("settings.storedHint")}
      ...
      <p className="mt-2 text-xs text-muted-foreground">{t("settings.storedHint")}</p>
    </form>
  );
        ...
        <div className="flex min-h-32 items-center justify-center rounded-lg border bg-card p-5 text-sm text-muted-foreground">
          {settingsLoadError ? (
            <div className="text-center">
              <div className="font-medium text-foreground">{t("common.serverError")}</div>
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6 p-6">
      ...
      <div className="space-y-2">
        ...
        <p className="max-w-3xl text-sm text-muted-foreground">{t("settings.llmDesc")}</p>
      </div>
